...
{
    // 计算机械腿相对于起始端的末端坐标
    Pws[0] = fkine(Thetas(PI / 3, THETA_STAND_2, THETA_STAND_3));
    Pws[1] = fkine(Thetas(0, THETA_STAND_2, THETA_STAND_3));
    Pws[2] = fkine(Thetas(-PI / 3, THETA_STAND_2, THETA_STAND_3));
    Pws[3] = fkine(Thetas(2 * PI / 3, THETA_STAND_2, THETA_STAND_3));
    Pws[4] = fkine(Thetas(PI, THETA_STAND_2, THETA_STAND_3));
    Pws[5] = fkine(Thetas(4 * PI / 3, THETA_STAND_2, THETA_STAND_3));
    // 默认站立坐标，这里copy一份
...
static Thetas ikine(Position3 &pos)
{
    ...
    static double R, Lr, alpha_r, alpha1, alpha2;
    pos1 = pos;
...
double Gait_prg::move_point()
{
    double fun,m_velocity;
 ...
 *@param height 机器人的高度
 */
void Gait_prg::set_height(double height)
{
    for (int i = 0; i < 6; i++)
    {
...
void Gait_prg::CEN_and_pace_cal()
{
    ...
    // 计算圆心模长
    double module_CEN = K_CEN / velocity.omega * sqrt(pow(velocity.Vx, 2) + pow(velocity.Vy, 2));
    ...
    if (velocity_s.Vx >= 0)
        ...
        CEN.x = -sqrt(pow(module_CEN, 2) / (1 + pow(velocity.Vx, 2) / pow(velocity.Vy, 2)));
    // 计算步伐大小
    double module_speed = pow(pow(abs(velocity.Vx), 3) + pow(abs(velocity.Vy), 3) + pow(abs(velocity.omega), 3), 1.0f / 3);
    //if (module_speed > MAX_SPEED)
    //    module_speed = MAX_SPEED; // 限制速度
    R_pace = KR_2 * module_speed;
    //// 计算步伐时间
    //if (R_pace > MAX_R_PACE)
    //    this->pace_time = uint32_t(MAX_SPEED / (R_pace / MAX_R_PACE)); // 若超过最大步伐大小则缩小步伐时间
    //else
    //    this->pace_time = MAX_SPEED; // 若小于最大步伐大小则固定步伐时间
    //if (R_pace > MAX_R_PACE)
 ...
 * 步态规划
 */
double points_z[6][N_POINTS]; //用于调试
void Gait_prg::gait_proggraming()
{
    Position3 Vec_CEN2leg_ends[6];    // 圆心到腿部末端的向量
    static double angle_off[6];        // 圆心与机械腿末端的夹角
    static double norm_CEN2legs[6];    // 圆心到机械腿末端的模长
    static double Rp_ratios[6];        // 各个机械腿步态规划的大小比例
    ...
    double max_norm_CEN2legs = 0;
    for (int i = 0; i < 6; i++)
        if (norm_CEN2legs[i] > max_norm_CEN2legs)
            max_norm_CEN2legs = norm_CEN2legs[i]; // 选出最大模长

    static double R_paces[6]; // 各个机械腿的步长
    for (int i = 0; i < 6; i++)
    {
        Rp_ratios[i] = norm_CEN2legs[i] / max_norm_CEN2legs; // 计算各个机械腿步态规划的大小比例
        R_paces[i] = Rp_ratios[i] * R_pace;                  // 计算各个机械腿步态的大小
    }
    double d_theta = 2 * R_paces[0] / norm_CEN2legs[0]; // 计算机械腿走一步绕圆心拐的角度，随便拿一组数据来算就行
    double step_size = d_theta / (N_POINTS / 2);

    /*********先对腿1，3，5做步态规划***********/
    static double angle_t;   // 用于计算该点的角度
    static double y_temp;    // 用于计算z轴高度的临时变量
    ...
    for (int i = 0; i < 5; i += 2)
    {
        ...
        else // 10-19，画上半圆
        {
            ...
            //else
            //point.z = sqrt(pow(R_pace, 2) - pow(y_temp, 2)) * Rp_ratios[i]*Kz + Pws[i].z;
            y_temp = (double)(LegControl_round - N_POINTS / 2) / (double)(N_POINTS / 2) * PI;
        ...
        points_z[i][LegControl_round] = point.z;
    }
    ...
    for (int i = 1; i <= 5; i += 2)
    {
        if (LegControl_round < N_POINTS / 2) // 0-9, 画上半圆
        {
            ...
            //else
            //point.z = sqrt(pow(R_pace, 2) - pow(y_temp, 2)) * Rp_ratios[i]*Kz + Pws[i].z;
            y_temp = (double)(LegControl_round) / (double)(N_POINTS / 2.) * PI;
            point.z = R_pace * sin(y_temp) * Kz * Rp_ratios[i] + Pws[i].z;
        }
        else // 10-19, 画下半圆
        {
    ...
    //std::cout << points_z[0][LegControl_round] << ":" << points_z[1][LegControl_round] << "\n";
}
